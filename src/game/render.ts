...
import { brandAspect, brandImage } from "./brand";
...
export function renderStatic(ctx: CanvasRenderingContext2D) {
  ...
  // header strip
  label(ctx, "NURSES STATION", STATION.x + STATION.w / 2, STATION.y + 23, 12, "#a9dcf7");

  // mop + bucket parked at the left end (pick-up point for spills)
  ctx.strokeStyle = "#c9a978";
  ctx.lineWidth = 4;
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(STATION.x + 26, STATION.y + 38);
  ctx.lineTo(STATION.x + 30, STATION.y + 78);
  ctx.stroke();
  ctx.lineCap = "butt";
  ctx.fillStyle = "#dfe9ef";
  rr(ctx, STATION.x + 22, STATION.y + 76, 18, 11, 5);
  ctx.fill();
  ctx.fillStyle = "#7fd7ff";
  rr(ctx, STATION.x + 20, STATION.y + 90, 20, 12, 4);
  ctx.fill();

  // key rack at the right end
  ctx.fillStyle = "#1b3346";
  rr(ctx, STATION.x + STATION.w - 42, STATION.y + 38, 28, 46, 5);
  ctx.fill();
  ctx.strokeStyle = "#b061ff";
  ctx.lineWidth = 2;
  for (let i = 0; i < 2; i++) {
    ctx.beginPath();
    ctx.arc(STATION.x + STATION.w - 33 + i * 10, STATION.y + 54, 4, 0, Math.PI * 2);
    ctx.stroke();
  }
  ctx.strokeStyle = "#b061ff";
  ctx.beginPath();
  ctx.moveTo(STATION.x + STATION.w - 33, STATION.y + 58);
  ctx.lineTo(STATION.x + STATION.w - 33, STATION.y + 70);
  ctx.stroke();

  // ---- brand plaque on the desk front
  const plq = { x: STATION.x + 50, y: STATION.y + 32, w: 240, h: 66 };
  ctx.fillStyle = "rgba(6,18,28,0.35)";
  rr(ctx, plq.x + 3, plq.y + 4, plq.w, plq.h, 8);
  ctx.fill();
  ctx.fillStyle = "#ffffff";
  rr(ctx, plq.x, plq.y, plq.w, plq.h, 8);
  ctx.fill();
  ctx.strokeStyle = "#8fb4c8";
  ctx.lineWidth = 1.5;
  ctx.stroke();
  ...
  const padX = 14;
  const padY = 9;
  const availW = plq.w - padX * 2;
  const availH = plq.h - padY * 2;
  // contain-fit using the artwork's real measured aspect, so any logo shape fits
  const ar = brandAspect();
  const lw = Math.min(availW, availH * ar);
  const lh = lw / ar;
  const lx = plq.x + (plq.w - lw) / 2;
  const ly = plq.y + (plq.h - lh) / 2;
  if (brand) {
    ctx.drawImage(brand, lx, ly, lw, lh);
  } else {
    // placeholder until the artwork resolves; re-rendered on load
    ctx.fillStyle = "#e8eef2";
    rr(ctx, lx, ly, lw, lh, 4);
    ctx.fill();
  }
