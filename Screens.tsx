...
import { MiracleLogo } from "./BrandLogo";
import type { Hud } from "../game/engine";
import type { ScoreEntry } from "../hooks/useHighScores";

export function Logo({ small = false }: { small?: boolean }) {
  return (
    <div className={small ? "flex flex-col gap-1.5" : "flex flex-col gap-2.5"}>
      <MiracleLogo className={small ? "h-7" : "h-11"} />
      <div className="flex items-baseline gap-2 border-t border-white/10 pt-1.5">
        <span className={`font-bold tracking-wide text-white ${small ? "text-lg" : "text-2xl"}`}>
          Care<span className="text-[#00AEEF]">Call</span>
        </span>
        <span
          className={`font-semibold text-slate-500 ${small ? "text-[7px]" : "text-[9px]"}`}
